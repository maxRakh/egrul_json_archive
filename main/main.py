...
def check_okved_valid(okved_for_check: Union[str, int]) -> None:
    ...
    if not pattern.match(str(okved_for_check)):
        raise ValueError("Необходимо правильно ввести группировку или конкретный ОКВЭД")
...
def get_egrul_data_from_file(file_path: str, okved_group: Union[str, int], region: str) \
    ...
    после чего возвращает список словарей с компаниями в соотвествии с заданными параметрами.
    """
...
def insert_data_to_database(companies_data: List[dict]) -> None:
    """Создает таблицу в БД, если она не существует и вносит в нее полученные из файла данные."""
    try:
...
def main():
    ...
    - region: Название города или региона, по которому осуществляется фильтрация компаний.
    - file_path: Путь к архивированному JSON-файлу, содержащему данные о компаниях.
    """
    okved = 62
    region = 'Хабаровск'
    file_path = '/Users/maxr/Downloads/egrul.json.zip'
    data = get_egrul_data_from_file(file_path=file_path, okved_group=okved, region=region)
